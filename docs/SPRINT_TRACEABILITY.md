...
| 5 — polish, support and accessibility | Users need flexible booking changes, portable local media, predictable management actions, visible support state and an inclusive portal. | Reschedule/cancellation preview; cash-at-pickup selection and owner receipt reconciliation; strict support workflow; licensed local multi-image bike galleries; synthetic demo avatars/KYC/evidence; relative media paths resolved by web and Flutter; responsive shadcn/Base UI action menus, confirmations, profile popover and Sonner feedback; dark mode, skip link, keyboard focus and reduced motion. |
...
Demo media provenance and every local filename are documented in
[DEMO_MEDIA_ATTRIBUTION.md](DEMO_MEDIA_ATTRIBUTION.md). Leelame and
`bike-buddy-demo` were consulted as interaction references only; Bike Buddy's
components, authorization rules, data model and implementation remain in this
repository.
