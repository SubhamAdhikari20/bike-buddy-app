...
const listWithPagination = async (
  ...
  countFn: (filter: Record<string, unknown>) => Promise<number>,
  filter: Record<string, unknown>,
...
const adminService = {
  ...
  async listUsers(query: Record<string, unknown>) {
    return listWithPagination(
      async (filter, sort, skip, limit) =>
        userRepository.list(filter, { sort, skip, limit }),
      userRepository.count,
      {},
      query,
    );
  ...
  async listOwners(query: Record<string, unknown>) {
    ...
    return listWithPagination(
      async (f, sort, skip, limit) =>
        ownerRepository.list(f, { sort, skip, limit }),
  ...
  async reviewKyc(renterId: string, status: "approved" | "rejected") {
    ...
    if (renter.kycStatus !== "pending") {
      throw new AppError(
        409,
        "This renter has no pending ID verification",
        "CONFLICT",
      );
    }
