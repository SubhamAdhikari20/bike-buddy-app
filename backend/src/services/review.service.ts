...
const recomputeBikeRating = async (bikeId: string) => {
  ...
  await bikeRepository.recomputeRating(
    bikeId,
...
const ensureReviewAccess = (
  ...
  if (referencesDocument(review.userId, auth.userId)) {
    return;
  }
...
const reviewService = {
  async createReview(
    ...
    if (!referencesDocument(booking.renterId, auth.profileId)) {
      throw new AppError(
        403,
    ...
    if (!referencesDocument(booking.bikeId, payload.bikeId)) {
      throw new AppError(
        400,
        "The selected bike does not belong to this booking",
        "BAD_REQUEST",
      );
    }
  ...
  async updateReview(
    ...
    await recomputeBikeRating(toDocumentId(review.bikeId) ?? "");
    return updated;
  },
