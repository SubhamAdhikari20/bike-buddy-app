...
const seed = async () => {
  ...
  const ownerUser1 = await UserModel.create({
  ...
  const ownerUser2 = await UserModel.create({
  ...
  const renterUser1 = await UserModel.create({
  ...
  const renterUser2 = await UserModel.create({
  ...
  await BikeModel.create([
    {
      ...
      category: "sports",
      description:
        "Well maintained Pulsar, serviced monthly. Two helmets included.",
      pricePerDay: 1500,
      pricePerHour: 200,
    ...
    },
    {
      ...
      location: {
        ...
        city: "Kathmandu",
        area: "Thamel",
        landmark: "Opposite Chhetrapati Sabha Griha",
        latitude: 27.7119,
        longitude: 85.3077,
      },
