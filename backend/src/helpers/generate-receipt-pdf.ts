...
export const streamReceiptPdf = (res: Response, data: ReceiptData) => {
  ...
  doc
    ...
    .fontSize(9)
    .font("Helvetica")
    .text("Motorbike rentals for Nepal - bikebuddy.com.np");
  doc.moveDown(1);
  ...
  if (data.breakdown.securityDeposit > 0) {
    line(
      "Refundable deposit (returned after the ride)",
      npr(data.breakdown.securityDeposit),
    );
  }
  ...
  if ((data.lateFeeAmount ?? 0) > 0) {
    line(
      "Recorded late-fee estimate (not charged)",
      npr(data.lateFeeAmount ?? 0),
    );
  }
  doc.moveDown(0.3);
  doc
  ...
  doc.text(
    `Payment method: ${data.paymentProvider ?? "N/A"} - status: ${data.paymentStatus} - mode: ${data.paymentMode}`,
  );
  doc.text(
    "No hidden fees. Questions? Contact Bike Buddy support from the app.",
  );
