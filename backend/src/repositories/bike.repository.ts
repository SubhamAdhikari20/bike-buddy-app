...
export const bikeRepository = {
  ...
  findById: (bikeId: string) =>
    BikeModel.findById(bikeId).populate(
      "ownerId",
      "fullName ownerStatus ownerVerificationDate bio profilePictureUrl",
    ),
  updateById: (bikeId: string, payload: Record<string, unknown>) =>
    BikeModel.findByIdAndUpdate(bikeId, payload, {
      new: true,
      runValidators: true,
    }),
