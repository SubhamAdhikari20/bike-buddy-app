...
export const reviewRepository = {
  ...
  findById: (reviewId: string) => ReviewModel.findById(reviewId),
  findByBookingId: (bookingId: string) => ReviewModel.findOne({ bookingId }),
  updateById: (reviewId: string, payload: Record<string, unknown>) =>
    ReviewModel.findByIdAndUpdate(reviewId, payload, {
      new: true,
      runValidators: true,
    }),
  ...
  listByBikeId: (bikeId: string, skip: number, limit: number) =>
    ReviewModel.find({ bikeId, isHidden: false })
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit)
      .populate("userId", "_id"),
  listByUserId: (userId: string, skip: number, limit: number) =>
    ReviewModel.find({ userId })
      ...
      .limit(limit)
      .populate("bikeId"),
  countByBikeId: (bikeId: string) =>
    ReviewModel.countDocuments({ bikeId, isHidden: false }),
